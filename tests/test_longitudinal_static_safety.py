...
PRODUCTION = Path(__file__).parents[1] / "src" / "paretodrive_longitudinal"
FORBIDDEN_IMPORTS = {
    "socket", "subprocess", "zipfile", "tarfile", "shutil", "send2trash", "urllib", "http",
    "requests", "aiohttp", "ftplib", "paretodrive_enrich", "paretodrive_preflight",
}
FORBIDDEN_CALLS = {
...
def test_longitudinal_has_no_source_content_executor_network_or_destructive_capability() -> None:
    violations: list[str] = []
    combined = ""
    ...
    assert "METADATA_UNCHANGED" in combined
    assert "dependency_claim" in combined and "dependency_evidence_coverage" in combined
