...
PRODUCTION = Path(__file__).parents[1] / "src" / "paretodrive_approval"
FORBIDDEN_IMPORTS = {
...
def test_approval_package_has_no_executor_content_read_or_destructive_capability() -> None:
    violations: list[str] = []
    combined = ""
    ...
    assert "execution_authorized\": False" in combined
    assert "archive_creation_authorized\": False" in combined
    assert "original_removal_authorized\": False" in combined
